...
検証が通ったら、AIが以下の観点で自動レビューを行い、レポートを提出する：

- **既存パターンとの一貫性** — コード規約・命名・構造が既存コードと合っているか
- **変更範囲の妥当性** — 実装計画で決めた範囲を超えていないか
- **エッジケース** — 未処理のエラーや境界条件がないか
- **テストカバレッジ** — 生成したテストが主要パスを網羅しているか
...
**要修正**の場合は修正を行い、Step 6（検証）からやり直す。
**OK**の場合はレポートを提示し、ユーザーの確認を待ってから次へ進む。
